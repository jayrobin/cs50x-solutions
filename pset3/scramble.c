...
bool lookup(string s)
{
    ...
    while(oldIndex != index)
    {
        ...
        index = min + ((max - min) / 2);
        //printf("%d, %d, %d\n", min, max, index);
        dictWord = dictionary.words[index];
        result = strncmp(s, dictWord.letters, LETTERS);
        ...
        {
            // word is earlier in dictionary
            max = index;
        }
        else
        {
            // word is later in dictionary
            min = index;
        }  
    }
...
void scramble(void)
{
    char rotatedGrid[DIMENSION][DIMENSION];
   
    for (int i = DIMENSION -1; i >= 0 ; i--) {
        for (int j = DIMENSION - 1; j >= 0; j--){
            rotatedGrid[i][j] = grid[DIMENSION - j - 1][i];
        }
    }
   
    // CAN I GET RID OF THE DOUBLE LOOP?
    for (int i = 0; i < DIMENSION; i++) {
        for (int j = 0; j < DIMENSION; j++){
